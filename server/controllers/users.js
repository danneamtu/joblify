...
export const getUser = async (req, res) => {
  ...
  try {
    ...
    console.log(err)
    res.status(409).json({ message: err.message || 'Users not found' })
  }
}
...
export const signup = async (req, res) => {
  ...
  try {
    ...
    const result = await Users.create({ email, password, familyName, givenName })
...
export const signin = async (req, res) => {
  ...
  try {
    ...
    res.status(400).send({ message: err.message || 'Sign in failed' })
  }
}
